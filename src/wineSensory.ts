...
export { GRAPE_CHARACTERS } from './grapeCharacter';
import { z } from 'zod';
...
import type { Character } from './grapeCharacter';
import { oakExposure, VESSELS } from './maturation';
...
export const tastingNotesSchema = z
  .object({
    ...
    aging: z.array(z.string().min(1).max(140)).min(1).max(4),
    fermentation: z.array(z.string().min(1).max(140)).min(1).max(3).optional(),
...
export function tastingProfile(
  ...
  let woodAge = 0;
  let dominantOak = 0;
  const fermentation = new Map<'oak' | 'steel', number>();
  ...
  for (const part of parts) {
    ...
    if (part.fermentation)
      fermentation.set(
        part.fermentation,
        (fermentation.get(part.fermentation) ?? 0) + part.ml,
      );
    ...
    if ('version' in maturation) {
      const exposure = oakExposure(
        maturation.vessel,
        maturation.weeks,
        part.fermentation === 'oak',
      );
      oakAroma += share * exposure * 0.18;
      oakAge += share * exposure;
      if (maturation.vessel !== 'steel') woodAge += share * maturation.weeks;
      if (maturation.oakDominant) dominantOak += share;
    } else if (maturation.vessel === 'oak') {
      oakAroma += share * (0.4 + maturation.weeks * 0.12);
      oakAge += share * maturation.weeks;
      woodAge += share * maturation.weeks;
    }
  }
  ...
  const tannin = Math.max(0, result.tannin - woodAge * 0.05);
  const texture =
    tannin < 0.5
      ? 'little tannin'
      : tannin < 2.5
  ...
  const palate = `${body}, with ${acidity} acidity and ${texture}. ${fruit}${woodAge >= 2 ? ' and a gently rounded oak texture' : ''}. ${finish}.${dominantOak >= 0.5 ? ' Oak dominates the fruit.' : ''}`;
    ...
    .map(([vessel, record]) => {
      const weeks =
        record.min === record.max
          ? String(record.min)
          : `${record.min}–${record.max}`;
      return `${percentage(record.ml / total)} ${vessel === 'steel' ? 'stainless steel' : VESSELS[vessel].name} · ${weeks} ${record.max === 1 && record.min === 1 ? 'week' : 'weeks'} of maturation${record.max === 0 ? ' after fermentation' : ''}.`;
    });
  if (unknown)
    aging.push(
      ...
      ? {
          fermentation: [
            ...[...fermentation]
              ...
              .map(
                ([vessel, ml]) =>
                  `${percentage(ml / total)} fermented in ${vessel === 'oak' ? 'French oak' : 'stainless steel'}.`,
              ),
            ...(parts.some((p) => !p.fermentation)
              ? [
                  `${percentage(parts.filter((p) => !p.fermentation).reduce((n, p) => n + p.ml, 0) / total)} fermentation history unrecorded.`,
                ]
              : []),
          ],
        }
