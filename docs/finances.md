...
Partial grape processing allocates harvest cost by kilograms used, retaining the remainder with the fresh grapes. Each batch then adds only its own tank and technique charges. Purchased research slots are investments. Large readable exports automatically use compact JSON when needed to stay within the save limit; migration checks the normalized size before autosave can replace an existing save.
