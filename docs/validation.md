...
## Uncapped Prestige and named tiers — September 7, 2026

- Replaced player-facing Reputation with Prestige and removed its 100-point ceiling. Sixteen named tiers run from Two-Buck Beginning to Pétrus Pantheon at 65,000; the score continues above the last tier. Retail and wholesale keep earning 0.04 and 0.01 per bottle. Economic influence tapers logarithmically above 100, preserving earlier balance. Wine scores keep their separate 100-point scale. Titles grant no cash or purchase unlocks.
- All 155 tests pass, including eight new Prestige scenarios covering every tier boundary, sales above 100 and 65,000, continued earnings after the last tier, exact cash accounting, penalties and downward tier changes, old/new save round trips, invalid scores, diminishing returns, formatting, and retained price, stock, and visitor limits. The production build, changed-code formatting check, and `git diff --check` pass.
- Existing isolated `localhost:5175` Domaine des Longs Rangs loaded with Prestige 10, $35, 630 L of reserves, and the same Year 2/week 9 state. No purchases or week advances were made. Authentic before/after Estate, Wine shop, Build, and Guide captures use the same paused estate and 1440 × 900 CSS viewport. Shop and Build captures cover the full page; Guide captures are scrolled to the bottom. All changed labels and explanations were visually inspected.
- The new dialog displays all 16 tiers, the current tier, the next threshold, earnings, and the separate wine-rating explanation. Keyboard Enter opens it, Tab stays within the dialog, End reaches the final tiers, and Escape closes it. A final keyboard check confirms focus returns to the Prestige resource after the page's inert state is removed. No application console errors were logged in the final QA tab.
- Inspected a matched Estate before/after pair and a supplementary tier-dialog capture at 390 × 844 CSS pixels. The resource bar is 19px taller to keep the tier name visible; the score, label, and dialog fit without horizontal overflow. Desktop and mobile dialog captures are after-only because the feature is new.
- A separate `localhost:5176` estate was started for synthetic high-score imports. Chrome's automated file chooser rejected local-file access, and the native-picker fallback was interrupted by activity in Chrome. No browser permissions were changed. Crossing the final tier and saving high scores have simulation coverage; this late-game browser import was not verified. The main port-5173 save was not used for gameplay QA.
- Original JPEG images, test/build logs, explicitly synthetic unused Prestige fixtures, scenario notes, and an unposted screenshot-comment template are retained in `/private/tmp/terroir-prestige-qa/`. No PR was requested.

## Ten-grape expansion — September 7, 2026

- Added Gamay, Carménère, Graciano, Petite Sirah / Durif, Pinot Gris / Pinot Grigio, Viognier, Albariño, Grüner Veltliner, Marsanne, and Roussanne. The catalog now has 34 grapes and 64 studies. Each new grape has individual research, climate/soil/yield traits, and blending affinities. Burgundy, Napa, Rioja, and Barossa gain relevant regional favorites; founding vines remain the same.
- All 163 tests pass, including full study → plant → harvest → ferment → reserve → bottle → sale loops and breeding trials for every addition. Coverage also checks 34-license saves, version-five and current save access, unfinished legacy collection rewards, new regional advantages, accent-insensitive search, proportion-sensitive Syrah–Viognier blending, and its red/white research gate. Existing exhaustive pairing symmetry, score bounds, rare 90-point wines, and plot-expansion checks include the enlarged catalog. Production build, changed-code formatting, and whitespace checks pass.
- Captured and inspected authentic before/after Research and Grape library screens at 1440 × 900 CSS pixels, plus the library at 390 × 844. Same paused Burgundy estate, Year 1/week 6, $12,500, 30 knowledge, two plantable grapes. All ten names were found through the library search, including ASCII spellings and alternate names such as Durif and Pinot Grigio.
- Inspected all four updated onboarding region previews at 1440 × 900 CSS pixels. Both sets use the default Domaine Bellevue name and the same selection sequence. Before captures use localhost:5177; after captures use a fresh 127.0.0.1:5177 Chrome tab to preserve the existing estate. Browser zoom/image scaling differs between these onboarding sets (1600px-wide before images versus 1440px-wide after images); they are content comparisons rather than pixel-identical comparisons.
- Chrome QA later reported cross-tab save updates. Gameplay verification therefore used a separate hidden in-app-browser estate at 127.0.0.1:5177. Through normal controls, completed Vine science, paid $3,400 and 70 knowledge for Gamay's ten-week local study, reloaded after four weeks to verify six remained, completed the study, and planted Gamay on Orchard field for $510. Reload retained the planted grape, three plantable varieties, and $3,870 at Year 3/week 4. No gameplay actions were taken against the main port-5173 save.
- Supplementary mobile inspections show the complete Pinot Gris / Pinot Grigio study row at 390 × 844 and Grüner Veltliner at 360 × 844. Costs, prerequisites, duration, and affordability messaging fit without horizontal document overflow. The final gameplay tab recorded no console errors. Temporary viewport overrides were reset.
- Original JPEG images, test/build logs, search results, scenario notes, and a prepared unposted evidence template are in `/private/tmp/terroir-ten-grapes-qa/`. No PR or deployment was requested during implementation.
